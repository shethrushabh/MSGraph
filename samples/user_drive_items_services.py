...
pprint(drive_item_services.get_drive_item(drive_id=DRIVE_ID, item_id=DRIVE_ITEM_ID))

# Grab a Drive Item, by path.
pprint(
    drive_item_services.get_drive_item_by_path(
        drive_id=DRIVE_ID, item_path="/Career - Certifications & Exams"
    )
)

# Grab a Drive Item, for a specific user in a specific Drive.
pprint(drive_item_services.get_user_drive_item(user_id=USER_ID, item_id=DRIVE_ITEM_ID))

# Grab a Drive Item, by path for a specific user in a specific Drive.
pprint(
    drive_item_services.get_user_drive_item_by_path(
        user_id=USER_ID, item_path="/Career - Certifications & Exams"
    )
)

# Grab my Drive Item by ID.
pprint(drive_item_services.get_my_drive_item(item_id=DRIVE_ITEM_ID))
...
pprint(
    drive_item_services.get_group_drive_item(group_id=GROUP_ID, item_id=DRIVE_ITEM_ID)
)

# Grab a group Drive Item, by path.
pprint(
    drive_item_services.get_group_drive_item_by_path(
        group_id=GROUP_ID, item_path="/Career - Certifications & Exams"
    )
)
